...
**1. Build Image:**
```bash
docker build -t screensaver-ws .
```

**2. Run Container:**
```bash
docker run -d -p 8080:8080 --name screensaver-ws screensaver-ws
```

**3. View Logs:**
```bash
docker logs -f screensaver-ws
```

**4. Stop:**
```bash
docker stop screensaver-ws
docker rm screensaver-ws
```
...
```bash
# Check tunnel status
cloudflared tunnel info screensaver-ws
...
class WebSocketClient @Inject constructor() {
    ...
    companion object {
        ...
        // private const val SERVER_URL = "ws://10.253.137.132:8080"
    }
